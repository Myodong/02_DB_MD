...
-- EMPLOYEE 테이블에서 모든사원의 사번, 이름, 급여, 급여 *12(연봉) 조회
...
FROM DUAL;
-- 날짜에 +/- 연산 시 일 다위로 계산이 진행된다

----------------------------------------------------------------------------------------

-- <컬럼 별칭 지정>

-- SELECT 조회 결과의 집합인 RESULT SET에 출력되는 컴럼명을 지정
/*
...
SELECT SYSDATE -1 "하루 전", SYSDATE AS 현재시간, SYSDATE + 1 내일 -- AS 생략가능 , " " 띄어쓰기 가능
FROM DUAL;
...
--> " " 쌍따옴표 특수문자, 기호, 대소문자, 등을 구분하여 나타낼때 사용하는 표기법
...
-- 주의사항 1) DISTINCT 구문은 SELECT 마다 딱 한번 씩만 작성이 가능하다
...
SELECT EMP_ID, EMP_NAME, SALARY
FROM EMPLOYEE
WHERE SALARY  NOT BETWEEN 300000 AND 6000000;


/* 날짜(DATE)에 BETWEEN 이용하기*/
-- EMPLOYEE 테이블에서 입사일이 1990-01-01 ~ 199-12-31 사이인 직원의
-- 이름, 입사일 조회
...
--EMPLOYEE 테이블에서 전화번호가 010으로 시작않는 사원의
-- 사번, 이름, 전화번호 조회
SELECT EMP_ID, EMP_NAME, PHONE
FROM EMPLOYEE
...
-- LIKE의 ESCAPE OPTION : 일반 문자로 처리할 '_' / '%' 앞에 아무 특수기호를 첨부해서 구분하게함
SELECT EMP_NAME, EMAIL
FROM EMPLOYEE
...
-- EMPLOYEE 테이블에서 
-- 이메일 '_'앞이 4글자 이면서 
-- 부서코드가'D9' 또는 'D6'이고 --> AND가 OR 보다 우선순위가 높다, ()가 사용가능
...
/* NOT IN*/
-- EMPLOYEE 테이블에서
-- 부서ㅗ드가 D1, D6, D9이 아닌 사원의
-- 사번, 이름, 부서코드 조회
SELECT EMP_ID, EMP_NAME, DEPT_CODE
FROM EMPLOYEE
WHERE DEPT_CODE NOT IN('D1','D6','D9') --12명
OR DEPT_CODE IS NULL; -- 부서코드 없는 2명 (NULL 처리 연산자)
 ...
 ** SELECT문 해석 시 가장 마지막에 해석된다!!!
 ...
 1. FROM 절
 2. WHERE절
 4. ORDER BY 컬럼명 | 별칭 | 컬럼 순서 [ASC | DESC]  [NULLS FIRST | LAST] // [] 대괄호 생략가능
 									  (오름/내림차순)  (앞정렬/뒤정렬)
...
SELECT EMP_ID, EMP_NAME, SALARY
FROM EMPLOYEE
ORDER BY SALARY; -- ASC가 기본값 안쓰면 ASC 적용
...
ORDER BY 3 DESC; -- 3: SELECT 에서  SALARY리가 3번째에 있다 는 의미 
...
FROM EMPLOYEE
ORDER BY DEPT_CODE ASC, SALARY DESC; --ASC는 기본값으로 안써도 됨
