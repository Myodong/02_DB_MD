...
SELECT  DEPARTMENT_NAME|| '의 정원은 ' || COUNT(*)||' 명 입니다.' "학과별 정원"
FROM TB_DEPARTMENT D
JOIN  TB_STUDENT S ON(D.DEPARTMENT_NO = S.DEPARTMENT_NO)
WHERE S.DEPARTMENT_NO IS NOT NULL
...
누구인가?(국문학과의 '학과코드'는 학과 테이블(TB_DEPARTMENT)을 조회해서 찾아내도록)*/
SELECT STUDENT_NAME
FROM TB_DEPARTMENT D
JOIN  TB_STUDENT S ON(D.DEPARTMENT_NO = S.DEPARTMENT_NO)
...
그 대상자들의 학번이 다음과 같을 때 대상자 들을 찾는 적절한 SQL 구문을 작성하시오.
A513079, A513090, A513091, A513110, A5513119 */
SELECT STUDENT_NAME
FROM TB_DEPARTMENT D
JOIN  TB_STUDENT S ON(D.DEPARTMENT_NO = S.DEPARTMENT_NO)
...
/*
 8. 수강신청을 하려고 한다. 선수과목 여부를 확인해야하는데, 선수과목이 존재하는
...
/*
 9. 춘 대학에는 어떤 계열(CATEGORY)들이 있는지 조회해보자*/
SELECT DISTINCT CATEGORY 
FROM TB_DEPARTMENT;

/*
 10. 02학번 전주 거주자들의 모임을 만들려고한다. 휴학한 사람들은 제외한 재학중인
 학생들의 한번, 이름, 주민번호를 출력하는 구문을 작성하시오*/
...
/*
 ...
 순으로 표시하는 SQL 문장을 작성하시오(단, 헤더는 학번, 이름, 입학년도 가 표시되도록 한다)*/
...
/*
 ...
 문장의 결과 값이 예상과 다르게 나올 수 있다. 원인이 무엇일지 생각해볼것)*/
...
/*
 ...
 2000년 이후 출생자는 없으며 출력 헤더는 "교수이름", "나이"로한다. 나이는 '만'으로 계산한다.)*/
SELECT PROFESSOR_NAME "교수이름",
CEIL (MONTHS_BETWEEN(SYSDATE ,TO_DATE(SUBSTR(PROFESSOR_SSN,1, INSTR(PROFESSOR_SSN,'-')-1),'RRMMDD'))/12) "나이" 
FROM TB_PROFESSOR
WHERE SUBSTR(PROFESSOR_SSN, 8, 1)='1' 
ORDER BY PROFESSOR_SSN  DESC;
-- R패턴 두번쓰는건 안됨 왜 두번이냐 TO-DATE가 기본적으로 R패턴임
-- R패턴 사용해도 40년대는 2000년생으 됨 , 만으로 나이 못구함
...
/*
 5. 춘 기술대학교의 재수생 입학자를 구하려고 한다. 어떻게 찾아낼 것인가? 이떄,
...
SELECT TO_CHAR(LAST_DAY('2020-12-01')-6,'YYYY"년"MM"월"DD"일" (DY)') "2020년 크리스마스"
FROM DUAL; 
...
/*
 9. 학번이 A517178인 한아름 학생의학점 총 평점을 구하는 SQL 문을 작성하시오.
 단, 이때 출력 화면의 헤더는 "평점" 이라고 찍히게 하고, 점수는 반올림하여 소수점
 이하 한 자리까지만 표시한다. */     
...
/*
 ...
 작성하시오 */
SELECT DEPARTMENT_NO 학과코드명, SUM(DECODE(ABSENCE_YN, 'Y',1,0)) "휴학생수" 
FROM TB_STUDENT
GROUP BY DEPARTMENT_NO
ORDER BY DEPARTMENT_NO;

SELECT DEPARTMENT_NO 학과코드명, COUNT(DECODE(ABSENCE_YN, 'Y',1)) "휴학생수" 
FROM TB_STUDENT
GROUP BY DEPARTMENT_NO
ORDER BY DEPARTMENT_NO;
...
/*
14. 춘 대하교에 다니SELECT STUDENT_NAME 동일는 동명이인 학생들의 이름을 찾고자 한다. 어떤 SQL 문장을 사용하면 가능하겠는가?*/
이름, COUNT(*) "동명인 수"
FROM TB_STUDENT
...
을 구하는 SQL 문을 작성하시오. (단, 평점은 소수점 1자리까지만 반올림하여 표시한다.)*/
...
/*
 1. 학생이름과 주소지를 표시하시오. 
 단, 출력헤더는 "학생이름","주소지"로 하고 정렬은 이름으로 오름차순 표시하도록 한다.*/
