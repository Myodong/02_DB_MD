...
SELECT INSTR('AABAACAABBAA','B') --3번째 
FROM DUAL;

-- 문자열을 5번째 문자 부터 검색하여 첫 번째 B 위치 조회
SELECT INSTR('AABAACAABBAA','B', 5) --9번째
FROM DUAL;
...
SELECT  EMP_NAME, SUBSTR(EMAIL, 1 , INSTR (EMAIL, '@')-1) AS "아 이 디"
FROM EMPLOYEE;
...
FROM DUAL; -- LEADING 앞쪽 제거

SELECT TRIM(LEADING '#' FROM'#####안녕#####')
FROM DUAL; -- TRAILING 뒤쪽 제거

SELECT TRIM(BOTH '#' FROM'#####안녕#####')
FROM DUAL; -- BOTH 양쪽 제거 (기본 값 안적어도됨)
...
SELECT ROUND(123.456, 1) FROM DUAL; -- 23,5 소수점 두 번째 자리에서 반올림
									--> 두번째 자리에서 반올림해서 소수점 한자리만 표현
...
SELECT ROUND(123.456, -2) FROM DUAL;-- 100, 10 자리에서 반올림해서 100 자리부터 표현						
...
SELECT TO_CHAR(1234) FROM DUAL;-- 1,234 -> '1234' 숫자 문자로 변환
SELECT TO_CHAR(1234,'99999') FROM DUAL; -- ' 1234' 앞 한칸 빈칸
SELECT TO_CHAR(1234,'00000') FROM DUAL; -- '01234' 빈칸 0넣기
...
SELECT TO_DATE('2022-09-02') FROM DUAL; -- 문자를 DATE 타입으로 변환
SELECT TO_DATE(20220902) FROM DUAL; -- 숫자를 DAYE 타입으로 변환
...
-- 직원의 성별 구하기(남:1 / 여: 2)
SELECT EMP_NAME AS 이름, DECODE(SUBSTR(EMP_NO,8,1), '1','남성','2','여성' ) 성별
FROM EMPLOYEE;

-- 직원의 급여를 인상하고자 한다.
-- 직급 코드가 J7인 직원들은 20%인상
-- 직급 코드가 J6인 직원들은 15%인상
-- 직급 코드가 J5인 직원들은 10%인상
-- 그외 직급은 5%인상
SELECT EMP_NAME, JOB_CODE, SALARY,
	DECODE(JOB_CODE, 'J7','20%','J6','15%','J6','10%','5%') 인상률, 
	DECODE(JOB_CODE, 'J7',SALARY *1.2,'J6',SALARY*1.15,'J6',SALARY*0.1,SALARY*1.05) "인상된 급여"
FROM EMPLOYEE;
