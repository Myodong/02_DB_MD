...
-- DML(Data Manipulation Language) : 테이터 조작 언어, 테이블에 데이터를 삽입, 수정, 삭제하는 언어
...
-- 데이터베이스 내에서 모든 데이터는 테이블을 통해서 저장된다. (중요)
...
-- ** JDBC에서 날짜를 입력 받았을 떄 삽입하는 방법 **
-- '2022-09-13 17:33:27'
INSERT INTO "MEMBER" VALUES('MEM05','PASS05','김길동','930303-1333333', TO_DATE('2022-09-13 17:33:27', 'YYYY-MM-DD HH24:MI:SS'));
																			--	?(PLACE HOLDER)
--  데이터  삽입 확인
SELECT * FROM "MEMBER";
COMMIT;
...
-- ORA-01400: NULL을 ("KH_KHD"."USER_USED_NN"."USER_NO") 안에 삽입할 수 없습니다
--------------------------------------------------------------------------------------------------------------------
...
-- ORA-00001: 무결성 제약 조건(KH_KHD.USER_ID_U)에 위배됩니다
...
-- ORA-00001: 무결성 제약 조건(KH_KHD.USER_ID_NAME_U)에 위배됩니다
...
--> 기본키 중복으로 오류
-- ORA-00001: 무결성 제약 조건(KH_KHD.USER_NO_PK)에 위배됩니다

INSERT INTO USER_USED_PK
VALUES(NULL, 'user03', 'pass03', '유관순', '여', '010-9999-3131', 'yoo123@kh.or.kr');
--> 기본키가 NULL 이므로 오류
-- ORA-01400: NULL을 ("KH_KHD"."USER_USED_PK"."USER_NO") 안에 삽입할 수 없습니다
...
-- ORA-00001: 무결성 제약 조건(KH_KHD.PK_USERNO_USERID)에 위배됩니다
...
-- ORA-01400: NULL을 ("KH_KHD"."USER_USED_PK2"."USER_NO") 안에 삽입할 수 없습니다
...
-- ORA-02291: 무결성 제약조건(KH_KHD.GRADE_CODE_FK)이 위배되었습니다- 부모 키가 없습니다
...
WHERE GRADE_CODE = 30;
-- ORA-02292: 무결성 제약조건(KH_KHD.GRADE_CODE_FK)이 위배되었습니다- 자식 레코드가 발견되었습니다
...
-- ORA-02290: 체크 제약조건(KH_KHD.GENDER_CHECK)이 위배되었습니다
...
PNO VARCHAR2(20) CONSTRAINT NN_PNO PRIMARY KEY,    -- 문제점 부분 
GENDER VARCHAR2(3) CONSTRAINT CK_GENDER CHECK(GENDER IN('남','여')),
PHONE VARCHAR2(20) ,
ADDRESS VARCHAR2(100),
STATUS  VARCHAR2(1) CONSTRAINT CK_STATUS NOT NULL CHECK(STATUS IN('Y','N'))
);/*PK가 UNIQUE, NOT NULL 의 특징을 모두 가지고 있지만, 
테이블 내에서 한 번만 지정할 수 있어서
적절한 컬럼을 제외하고는 사용하시면 안됨*/
