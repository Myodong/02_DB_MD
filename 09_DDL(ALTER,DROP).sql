...
ADD CONSTRAINT DEPT_COPT_TITLE_U UNIQUE(DEPT_TITLE);

-- DEPT_COPY의 DEPT_TITLE 컬럼에 UNIQUE 삭제
ALTER TABLE DEPT_COPY
DROP CONSTRAINT DEPT_COPT_TITLE_U ;
...
-- ORA-12899: "KH_KHD"."DEPT_COPY"."DEPT_ID" 열에 대한 값이 너무 큼(실제: 3, 최대값: 2)
...
-- ORA-02291: 무결성 제약조건(KH_KHD.SYS_C008417)이 위배되었습니다- 부모 키가 없습니다

-- TB1 삭제
DROP TABLE TB1;
-- ORA-02449: 외래 키에 의해 참조되는 고유/기본 키가 테이블에 있습니다
--> 해결방법	
-- 1) 자식, 부모 테이블 순서로 삭제 (자식도 삭제됨)
-- 2) ALTER를 이용해서 FK 제약조건 삭제후 TB1 삭제  (번거로움 2번 삭제)
...
--> DDL은 수행시 존재하고있는 트랜잭션을 모두 DB에 강제 COMMIT 시킴
