...
--JOIN DEPARTMENT ON (연결고리 역할의 컬럼);
...
--JOIN JOB ON(JOB_CODE = JON_CODE);
--JON_CODE가 어디꺼인지 알수없음
 ...
 * -> 연결에  사용된 컬럼의 값이 일치하지 않으면 조회 겨로가에 포함되지 않는다.
 * 
 */
...
SELECT EMP_NAME, DEPT_TITLE
FROM EMPLOYEE
CROSS JOIN DEPARTMENT; --207행 == 23 * 9 == 270
...
-- ANSI 표준
SELECT E1.EMP_ID, E1.EMP_NAME, NVL(E1.MANAGER_ID, '없음'), NVL(E2.EMP_NAME, '_')
FROM EMPLOYEE E1
...
-- 사원이름, 부서명, 지역명, 조회
-- EMPLOYEE ,DEPARTMENT, LOCATION  테이블3개
...
-- SELF JOIN 사용
SELECT E1.EMP_NAME 사원명, E1.DEPT_CODE 부서코드, E2.EMP_NAME 
FROM EMPLOYEE E1
